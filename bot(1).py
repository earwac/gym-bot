...
Commands (slash):
  ...
  /add_day          -> (admin) manually log a gym day for someone, any date
...
@bot.tree.command(description="(Admin) Manually log a gym day for someone on any date")
@app_commands.describe(
    member="Who to log a gym day for",
    day="Date as MM/DD or MM/DD/YYYY (defaults to this year). Leave blank for today.",
)
@app_commands.checks.has_permissions(administrator=True)
async def add_day(interaction: discord.Interaction, member: discord.Member, day: str = None):
    if day is None or day.strip() == "":
        ...
        for fmt in ("%m/%d/%Y", "%m/%d/%y", "%m/%d", "%Y-%m-%d"):
            try:
                ...
                if fmt == "%m/%d":
                    parsed = parsed.replace(year=local_today().year)
        ...
        if target_day is None:
            await interaction.response.send_message(
                f"Couldn't understand the date **{raw}**. Try formats like `9/5`, `09/05/2026`, "
                f"or `2026-09-05`.",
                ephemeral=True,
            )
            return
