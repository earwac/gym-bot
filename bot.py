...
def effective_today() -> date:
    """Today, capped at CHALLENGE_END so week/owed math stops counting once
    the challenge is over (instead of racking up unlimited future weeks)."""
    t = date.today()
...
async def undo(interaction: discord.Interaction):
    removed = remove_day(interaction.guild_id, interaction.user.id, date.today())
    if removed:
